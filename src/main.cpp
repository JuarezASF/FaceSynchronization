            ...
            for (size_t f = 0; f < shapes[s].mesh.indices.size() / 3; f++) {
                ...
                if (attrib.normals.size() > 0) {
                    ...
                    if (shouldInvertNormals) {
                        for (int k = 0; k < 3; k++) {
                            n[0][k] = -1 * attrib.normals[3 * f0 + k];
                            n[1][k] = -1 * attrib.normals[3 * f1 + k];
                            n[2][k] = -1 * attrib.normals[3 * f2 + k];
                        }
                    } else {
                        for (int k = 0; k < 3; k++) {
                ...
                for (int k = 0; k < 3; k++) {
                    ...
                    attributes.push_back(c[0] * 0.5 + 0.5);
                    attributes.push_back(c[1] * 0.5 + 0.5);
                    attributes.push_back(c[2] * 0.5 + 0.5);
                }
            }
...
//                glBufferData(GL_ARRAY_BUFFER, attributes.size() * sizeof(float), &attributes.at(0),
//                             GL_STATIC_DRAW);
                drawObject.numTriangles = attributes.size() / 9 / 3;
...
int main(int argc, char **argv) {
    ...
    cvCreateTrackbar("neutro", "Control", &alpha_slider0, alpha_slider_max, onCallBack0);
    cvCreateTrackbar("bravo", "Control", &alpha_slider1, alpha_slider_max, onCallBack0);
    cvCreateTrackbar("feliz", "Control", &alpha_slider2, alpha_slider_max, onCallBack0);
    cvWaitKey(50);
    ...
    if (!LoadObjAndConvert(bmin, bmax, &drawObjects, initial, false)) {
        return -1;
    }
    if (!LoadObjAndConvert(bmin, bmax, &objectA, faceA, false)) {
        return -1;
    }
    if (!LoadObjAndConvert(bmin, bmax, &objectB, faceB, false)) {
        return -1;
    }
    if (!LoadObjAndConvert(bmin, bmax, &objectC, faceC, false)) {
        return -1;
    }
